...
function analyzePrompt(prompt, technique) {
  ...
  switch(technique) {
    case 'agent':
      const agentPhrases = ['инструкция', 'роль', 'задача', 'функция', 'инструмент', 'tool', 'api', 'действие'];
      const hasAgent = agentPhrases.some(phrase => lowerPrompt.includes(phrase));

      if (hasAgent) {
        feedback = `<strong style="color: var(--accent);">✓ Отлично!</strong> Ваш промпт содержит элементы для настройки агента.\n\n`;
        feedback += `<strong>Найденные элементы:</strong>\n`;
        agentPhrases.forEach(phrase => {
          if (lowerPrompt.includes(phrase)) {
            feedback += `• "${phrase}"\n`;
          }
        });
      } else {
        feedback = `<strong style="color: var(--warning);">⚠ Совет:</strong> Добавьте ключевые элементы агента:\n\n`;
        feedback += `• Роль и экспертиза\n`;
        feedback += `• Задачи и функции\n`;
        feedback += `• Доступные инструменты`;
      }
      break;

    case 'automation':
      const automationPhrases = ['триггер', 'workflow', 'автоматизация', 'интеграция', 'webhook', 'api', 'действие', 'условие'];
      const hasAutomation = automationPhrases.some(phrase => lowerPrompt.includes(phrase));

      if (hasAutomation) {
        feedback = `<strong style="color: var(--accent);">✓ Отлично!</strong> Ваш промпт содержит элементы автоматизации.\n\n`;
        feedback += `<strong>Найденные элементы:</strong>\n`;
        automationPhrases.forEach(phrase => {
          if (lowerPrompt.includes(phrase)) {
            feedback += `• "${phrase}"\n`;
          }
        });
      } else {
        feedback = `<strong style="color: var(--warning);">⚠ Совет:</strong> Для автоматизации добавьте:\n\n`;
        feedback += `• Триггер (когда запускать)\n`;
        feedback += `• Действия (что делать)\n`;
        feedback += `• Условия (если-то)`;
      }
      break;
