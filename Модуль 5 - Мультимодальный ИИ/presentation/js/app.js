...
function analyzePrompt(prompt, technique) {
  ...
  switch(technique) {
    case 'image':
      const imagePhrases = ['стиль', 'освещение', 'композиция', 'детали', 'фон', 'цвета', 'качество', 'реалистичн', 'фото', '4k', '8k'];
      const hasImage = imagePhrases.some(phrase => lowerPrompt.includes(phrase));

      if (hasImage) {
        feedback = `<strong style="color: var(--accent);">✓ Отлично!</strong> Ваш промпт содержит элементы для генерации изображений.\n\n`;
        ...
        feedback = `<strong style="color: var(--warning);">⚠ Совет:</strong> Добавьте детали для генерации:\n\n`;
        feedback += `• Стиль (фотореалистичный, иллюстрация, 3D)\n`;
        feedback += `• Освещение (мягкое, драматичное, студийное)\n`;
        feedback += `• Качество (4K, высокая детализация)`;
      }
      break;

    case 'video':
      const videoPhrases = ['движение', 'камера', 'переход', 'длительность', 'кадр', 'план', 'сцена', 'секунд'];
      const hasVideo = videoPhrases.some(phrase => lowerPrompt.includes(phrase));

      if (hasVideo) {
        feedback = `<strong style="color: var(--accent);">✓ Отлично!</strong> Ваш промпт содержит элементы для генерации видео.\n\n`;
        feedback += `<strong>Найденные элементы:</strong>\n`;
        ...
        feedback = `<strong style="color: var(--warning);">⚠ Совет:</strong> Для видео добавьте:\n\n`;
        feedback += `• Движение камеры (панорама, наезд)\n`;
        feedback += `• Длительность сцены\n`;
        feedback += `• Переходы между кадрами`;
      }
      break;
